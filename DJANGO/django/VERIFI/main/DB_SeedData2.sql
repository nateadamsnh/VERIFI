...
source_table        varchar(55) NOT NULL,                     -- name of source table (raw) imported
source_field_name   varchar(55) NOT NULL,                     -- columm name used to locate position.
operation           varchar(55) NOT NULL,                     -- operation to perform on the source field id, i.e. avg, 
group_id			      int			NOT NULL, 					              -- the id this rule belongs to if reused in another rule. 
parent_group_id		  int			NOT NULL DEFAULT 0,			          -- if this operation is used in another rule, the is the rule group it belongs to. 
group_field_name    varchar(55) NULL,   					            -- order of grouping to perform
operation_id        int         NOT NULL,                     -- group order of operation
-- order_id		      	int			NOT NULL, 					              -- if this operation is part of a larger set then this id indicates what order to apply to the larger set of operations. 
PRIMARY KEY (id)
);
...
INSERT INTO rules (source_table, source_field_name, operation, group_id, parent_group_id, group_field_name, operation_id) VALUES (
'RESULTS_PARENTThaiCompleteSurvey(v1).csv', '_age', 'AVERAGE', 2, 0, 'gender', 1);

insert INTO rules (source_table, source_field_name, operation, group_id, parent_group_id, group_field_name, operation_id) VALUES (
'view', 'Input_Farm_ID', 'PLOT', 3, 0, null, 1), 
('view', 'PGS_total_yield_daily', 'PLOT', 3, 0, NULL, 2);
